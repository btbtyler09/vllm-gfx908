...
_dynamo_config_logging_patched = False


def _json_safe_dynamo_config_value(value):
    if isinstance(value, set):
        return sorted(
            (_json_safe_dynamo_config_value(item) for item in value),
            key=repr,
        )
    if isinstance(value, (list, tuple)):
        return [_json_safe_dynamo_config_value(item) for item in value]
    if isinstance(value, dict):
        return {
            str(key): _json_safe_dynamo_config_value(item)
...
def _apply_dynamo_config_logging_patch():
    """Make Dynamo metrics logging tolerate callable-valued config entries."""
    global _dynamo_config_logging_patched
    if _dynamo_config_logging_patched:
        return

    if not is_torch_equal_or_newer("2.6"):
        _dynamo_config_logging_patched = True
        return
    ...
    def _get_dynamo_config_for_logging_json_safe():
        try:
            return original()
        except TypeError:
            config_dict = {
                key: _json_safe_dynamo_config_value(value)
                for key, value in dynamo_config.get_config_copy().items()
                if key not in blocklist
            }
            return json.dumps(config_dict, sort_keys=True)

    _get_dynamo_config_for_logging_json_safe._vllm_json_safe = True
