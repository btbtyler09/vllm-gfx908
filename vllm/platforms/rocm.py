...
if _ON_GFX908:
    _GFX908_DEFAULTS = {
        ...
        # GPTQ linear kernel selection: upstream's AutoGPTQ consolidation
        # routes "gptq" checkpoints through choose_mp_linear_kernel(), where
        # TritonW4A16/TritonW8A16/Conch outrank Exllama. On gfx908 the tuned
        # exllama gptq_gemm (Rounds 5/6) beats them at decode, and the Exllama
        # kernel carries the dual W8 dispatch (Triton MFMA at high M)
        # internally — so pin Exllama by disabling the registry entries above
        # it. A user-set VLLM_DISABLED_KERNELS overrides this default.
        "VLLM_DISABLED_KERNELS": (
            "TritonW4A16LinearKernel,TritonW8A16LinearKernel,ConchLinearKernel"
        ),
...
class RocmPlatform(Platform):
    ...
    def get_vit_attn_backend(
        ...
        # gfx908: AITER CK FA does not build/run on CDNA1 — keep it out.
        if rocm_aiter_ops.is_mha_enabled() and on_cdna() and not on_gfx908():
