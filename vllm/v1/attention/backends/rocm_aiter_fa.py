    ...
    def forward(
        ...
        if not attn_metadata.use_cascade:
            ...
            if num_decodes > 0:
                ...
                # Multi-token speculative decode path, or gfx908 (ll4mi
                # gated to gfx90a+, Triton PA decode exceeds MI100 64KB LDS
                # with block_size=528).
                from vllm.platforms.rocm import on_gfx908 as _on_gfx908

                if decode_max_query_len > 1 or _on_gfx908():
                    assert not rocm_aiter_ops.is_shuffle_kv_cache_enabled(), (
                        "Shuffle KV cache layout is not supported with "
                        "sliding window or speculative decoding "
                        "(multi-token decode)."
                    )
