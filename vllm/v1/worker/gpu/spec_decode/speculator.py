...
class DraftModelSpeculator(BaseSpeculator):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # Draft dtype, not target dtype: a DFlash drafter may run in bf16
        # under an fp16 target (see SpeculativeConfig draft dtype note); for
        # all other methods draft_model_config inherits the target dtype so
        # this is a no-op.
