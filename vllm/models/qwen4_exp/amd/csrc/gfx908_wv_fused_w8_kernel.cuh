...
// NB <= 3: the pre-change kernel, byte-for-byte from `git show HEAD`.  An `if constexpr (CHUNK)`
// inside a single kernel was NOT enough here -- it still cost +1.3..2.0 us per HC module at
// M = 1..3 (measured HEAD vs new through the real op, agents/moe_prep_hc/bench_head_vs_new.log),
// so the two shapes are two kernels and NB <= 3 gets HEAD's instruction stream unchanged.
...
// ---------------------------------------------------------------------------------------------
// NB > 3: the same GEMV with the activation staged in KCH-column chunks (LDS row n at n*KCH), so
// the [M, 10240] mix_down activation no longer has to fit 62 KB in one piece.  All 16 waves must
// reach the per-chunk barriers, hence no early return and an `active` guard on the writer.
template <int YTILE, int UNRL, int NB, bool GRP, bool F32LDS, int LPR, int KS, int EPI>
__global__ void __launch_bounds__(1024)
hc_w8_gemv_chunk_kernel(const int K, const int Nrows, const int8_t* __restrict__ W,
                  const float* __restrict__ S, const uint16_t* __restrict__ X,
                  uint16_t* __restrict__ C, const int wvPrGrp, const int rounds, const int CuCount,
                  const int G, const int gshift,
                  const uint16_t* __restrict__ XN, const int HD, const int R, const int HCn,
                  const int Xs, const int KCH) {
  constexpr int THRDS = 64, A_CHUNK = 16, KSTEP = LPR * A_CHUNK;  // k per wave-step
  constexpr int SUBS = THRDS / LPR, RPW = YTILE * SUBS;            // rows per wave
  constexpr int RG = 16 / KS;                                      // row groups per WG
  constexpr int LDS_BF16 = HW8_X_BYTES / 2;
  static_assert(KS == 1 || (NB * RPW <= 32 && NB * RPW <= THRDS), "split-K partial buffer");
  static_assert(EPI != 2 || KS == 1, "gate-mix epilogue needs the full row in one wave");
  constexpr bool CHUNK = true;   // this kernel is the chunked one (NB > 3 only)
  __shared__ __align__(16) unsigned char lds_raw[HW8_LDS_BYTES];
  float* sf = reinterpret_cast<float*>(lds_raw);
  uint16_t* sh = reinterpret_cast<uint16_t*>(lds_raw);
  float* red = reinterpret_cast<float*>(lds_raw + HW8_X_BYTES);
  ...
  const int tid = threadIdx.y * THRDS + threadIdx.x;
  const int total = NB * K; (void)total;
  const int lane = threadIdx.x, wave = threadIdx.y;
  const int rg = wave / KS, ks = wave % KS;
  const int sub = lane / LPR, kl = lane % LPR;
  const int Kslice = K / KS, kbeg = ks * Kslice, kend = kbeg + Kslice;
  ...
  float sc[YTILE][UNRL];
  // ---- weight stream: YTILE*UNRL dwordx4 nontemporal loads in flight per lane
  auto issue = [&](uint32_t k1, uint32_t mm, uint32_t khi) {
#pragma unroll
    for (int k2 = 0; k2 < UNRL; k2++) {
      uint32_t k_ = k1 + k2 * KSTEP + kl * A_CHUNK;
      uint32_t kc = min(k_, khi - A_CHUNK);
#pragma unroll
      for (int y = 0; y < YTILE; y++) {
        uint32_t row = min(mm + sub * YTILE + y, (uint32_t)(Nrows - 1));
        bw[y][k2] = hw8_loadnt(reinterpret_cast<const hw8_u32x4_t*>(W + (size_t)row * K + kc));
...
#pragma unroll
    ...
    auto run_range = [&](int kc0, uint32_t klo, uint32_t khi) {
        issue(klo, m, (uint32_t)khi);
        for (uint32_t k1 = klo; k1 < (uint32_t)khi; k1 += KSTEP * UNRL) {
          // dequant + fma in small k chunks to keep VGPR pressure low (64 VGPR budget at 4 waves/SIMD)
  #pragma unroll
          for (int k2 = 0; k2 < UNRL; k2++) {
            uint32_t k_ = k1 + k2 * KSTEP + kl * A_CHUNK;
  ...
  #pragma unroll
                for (int n = 0; n < NB; n++) {
                  float xv[XW];
                  const int lofs = n * (CHUNK ? KCH : K) + (int)(k_ - (CHUNK ? kc0 : 0)) + XW * jj;
                  if constexpr (F32LDS) {
                    const float4 a = *reinterpret_cast<const float4*>(sf + lofs);
                    xv[0] = a.x; xv[1] = a.y; xv[2] = a.z; xv[3] = a.w;
                  } else {
                    const hw8_u32x4_t u = *reinterpret_cast<const hw8_u32x4_t*>(sh + lofs);
                    xv[0] = hw8_bf16lo(u.x); xv[1] = hw8_bf16hi(u.x); xv[2] = hw8_bf16lo(u.y); xv[3] = hw8_bf16hi(u.y);
                    xv[4] = hw8_bf16lo(u.z); xv[5] = hw8_bf16hi(u.z); xv[6] = hw8_bf16lo(u.w); xv[7] = hw8_bf16hi(u.w);
                  }
  #pragma unroll
  ...
  #pragma unroll
          ...
          const uint32_t knext = k1 + KSTEP * UNRL;
          if (knext < (uint32_t)khi) issue(knext, m, (uint32_t)khi);
        }
    };

    if constexpr (!CHUNK) {
      if (active) run_range(0, (uint32_t)kbeg, (uint32_t)kend);
    } else {
    for (int kc0 = 0; kc0 < K; kc0 += KCH) {
      const int klen = (KCH < K - kc0) ? KCH : (K - kc0);
      __syncthreads();
      for (int i = tid * 8; i < NB * klen; i += 1024 * 8) {
        const int nn = (NB == 1) ? 0 : (i / klen);
        const int kk = i - nn * klen;
        const hw8_u32x4_t v =
            *reinterpret_cast<const hw8_u32x4_t*>(X + (size_t)nn * Xs + kc0 + kk);
        if constexpr (F32LDS) {
          float4 a = make_float4(hw8_bf16lo(v.x), hw8_bf16hi(v.x), hw8_bf16lo(v.y), hw8_bf16hi(v.y));
          float4 b = make_float4(hw8_bf16lo(v.z), hw8_bf16hi(v.z), hw8_bf16lo(v.w), hw8_bf16hi(v.w));
          *reinterpret_cast<float4*>(sf + nn * KCH + kk) = a;
          *reinterpret_cast<float4*>(sf + nn * KCH + kk + 4) = b;
        } else {
          *reinterpret_cast<hw8_u32x4_t*>(sh + nn * KCH + kk) = v;
        }
      }
      __syncthreads();
      if (!active) continue;
      const int klo = (kbeg > kc0) ? kbeg : kc0;
      const int khi = (kend < kc0 + klen) ? kend : (kc0 + klen);
      if (klo >= khi) continue;
      run_range(kc0, (uint32_t)klo, (uint32_t)khi);
    }
    }
    __builtin_amdgcn_sched_barrier(0);
    // ---- cross-lane reduction
...
#pragma unroll
    ...
    if constexpr (KS == 1) {
      if ((!CHUNK || active) && kl == LPR - 1) {  // lane 63 (and lane 31 when LPR == 32) writes its sub-rows
        const uint32_t row0 = m + sub * YTILE;
        if constexpr (EPI == 2) {
          // gate-mix over the YTILE(=HC) permuted rows of hidden index i
          if (row0 + YTILE <= (uint32_t)Nrows) {
