...
torch::stable::Tensor gptq_w8a16_repacked_gemm(
  ...
  STD_TORCH_CHECK(a.dim() == 2, "expected 2D activation tensor");
  STD_TORCH_CHECK(b_q_weight.dim() == 2,
                  "expected 2D repacked qweight tensor");
  STD_TORCH_CHECK(a.size(0) >= 1 && a.size(0) <= BLOCK_M_SIZE_MAX,
                  "gptq_w8a16_repacked_gemm only supports M in [1, 8]");
  STD_TORCH_CHECK(a.size(1) == b_q_weight.size(0),
  ...
  STD_TORCH_CHECK(size_k % groups == 0,
                  "K must be divisible by GPTQ group count");
  STD_TORCH_CHECK((size_k / groups) % 32 == 0,
                  "GPTQ group size must be a multiple of 32");
  STD_TORCH_CHECK(b_gptq_qzeros.size(1) == b_q_weight.size(1),
