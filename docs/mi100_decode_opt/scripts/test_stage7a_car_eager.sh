#!/usr/bin/env bash
# Stage 7a: prove CAR works in eager and breaks ONLY under graph capture.
# Mounts custom_all_reduce.py with the gfx908 bypass REMOVED so CAR fires
# under capture. We expect: corruption (!!!!!  output) post-warmup.
#
# Then we test a few quick mitigations:
#  - HIP_LAUNCH_BLOCKING=1 (serialize kernel launches)
#  - AMD_SERIALIZE_KERNEL=3 (force kernel completion before next)
#  - HIP_FORCE_QUEUE_PROFILING=0 (no-op probably)
#
# If any mitigation works → bug is a race condition, not stale IPC pointers.
set -eu

IMG="${1:-vllm-rocm-gfx908:latest}"
SERVED="qwen3.6-35b-8bit"
MODE="${2:-baseline}"  # baseline | hipblock | amdserial
...
case "$MODE" in
  baseline)
    ...
    ;;
  hipblock)
    EXTRA_ENV="--env HIP_LAUNCH_BLOCKING=1"
    LABEL=stage7a_hipblock
    ;;
  amdserial)
    EXTRA_ENV="--env AMD_SERIALIZE_KERNEL=3"
    LABEL=stage7a_amdserial
    ;;
  *)
...
LOG=/tmp/decode_opt/serve_${LABEL}.log
: > "$LOG"

echo "=== launch with no-bypass CAR overlay + MODE=$MODE ==="
...
echo "=== waiting for startup (up to 20min — HIP_LAUNCH_BLOCKING is slow) ==="
...
if /home/tyler/vllm-gfx908/docs/mi100_decode_opt/scripts/coherence.sh "$SERVED" ${LABEL}_pre; then
  echo "COHERENCE PRE: PASS  (NO CAR corruption observed yet)"
  COHERENCE_PRE=PASS
else
  echo "COHERENCE PRE: FAIL  (CAR corruption immediately)"
  COHERENCE_PRE=FAIL
fi
...
if [[ "$COHERENCE_PRE" == "PASS" ]]; then
  ...
  echo ""
  echo "=== post-bench coherence (drift check) ==="
  if /home/tyler/vllm-gfx908/docs/mi100_decode_opt/scripts/coherence.sh "$SERVED" ${LABEL}_post; then
    echo "COHERENCE POST: PASS (CAR works, no drift!)"
  else
    echo "COHERENCE POST: FAIL (drift under sustained load)"
  fi
fi
