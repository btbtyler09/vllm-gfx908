#!/usr/bin/env bash
# Stage 6a: fuse MoE gate + shared_expert_gate into one F.linear
# Saves 1 launch per layer × 40 layers ≈ 2ms / 10% TPOT
set -eu
...
: > /tmp/decode_opt/serve_stage6a.log

echo "=== launch $IMG with utils.py + rocm.py + qwen3_next.py overlays ==="
docker run -d --name decode_opt \
  ...
  -v /home/tyler/vllm-gfx908/vllm/model_executor/models/qwen3_next.py:/usr/local/lib/python3.12/dist-packages/vllm/model_executor/models/qwen3_next.py:ro \
  "$IMG" \
...
nohup docker logs -f decode_opt > /tmp/decode_opt/serve_stage6a.log 2>&1 &
TAILER_PID=$!
disown $TAILER_PID

echo "=== waiting for startup (up to 15min) ==="
for i in $(seq 1 180); do
  if grep -qE 'Application startup complete|Uvicorn running' /tmp/decode_opt/serve_stage6a.log 2>/dev/null; then
    echo "READY after ${i}x5s"
    break
  fi
  if ! docker ps --filter name=decode_opt -q | grep -q .; then
    echo "FAIL: container exited during startup"
    tail -50 /tmp/decode_opt/serve_stage6a.log
...
if ! /home/tyler/vllm-gfx908/docs/mi100_decode_opt/scripts/coherence.sh "$SERVED" stage6a_pre; then
  echo "FAIL: pre-bench coherence — fusion broke output"
  kill $TAILER_PID 2>/dev/null || true
  exit 2
fi
...
if ! /home/tyler/vllm-gfx908/docs/mi100_decode_opt/scripts/coherence.sh "$SERVED" stage6a_post; then
