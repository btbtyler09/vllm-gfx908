...
: > /tmp/decode_opt/serve_stage3.log
...
nohup docker logs -f decode_opt > /tmp/decode_opt/serve_stage3.log 2>&1 &
disown

echo "=== waiting for startup (up to 15min) ==="
for i in $(seq 1 180); do
  if grep -qE 'Application startup complete|Uvicorn running' /tmp/decode_opt/serve_stage3.log 2>/dev/null; then
    echo "READY after ${i}x5s"
    break
  fi
  if ! docker ps --filter name=decode_opt -q | grep -q .; then
    echo "FAIL: container exited during startup"
    tail -50 /tmp/decode_opt/serve_stage3.log
    exit 1
  fi
  sleep 5
done
...
sleep 2
LM_HEAD_HITS=$(grep -c 'GEMM_A16W16: x=(1, 2048) w=(62080, 2048)' /tmp/decode_opt/serve_stage3.log || true)
...
if ! /tmp/decode_opt/coherence.sh "$SERVED" stage3_pre; then
...
if ! /tmp/decode_opt/coherence.sh "$SERVED" stage3_post; then
